...
describe('showService', () => {
  describe('getEpisodes', () => {
    ...
    test('when API return episode list return the episodes grouped by season', async () => {
      ...
      const firstSeason = groupedEpisodes.seasons[1];
      const secondSeason = groupedEpisodes.seasons[2];

      expect(firstSeason[0]).toEqual(showMocks.episode1);
      expect(firstSeason[1]).toEqual(showMocks.episode2);

      expect(secondSeason[0]).toEqual(showMocks.episode22);
      expect(secondSeason[1]).toEqual(showMocks.episode23);
    });
  });
});
